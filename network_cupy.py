...
def SIGMOID(inp):
    return cu.exp(-cu.logaddexp(0, -inp))

def LINEAR_DERIVE(act):
    return cu.ones(act.shape)
...
class NeuralNet(nb.NeuralNet):
    ...
    def __init__(self,inputs,outputs,hiddens=[],activations=[LINEAR]):
        ...
        for nex in layers:
            ...
            self._thetas.append(cu.random.rand(prev+1,nex)-0.5)
            # 
            prev = nex
    ...
    def ForwardProp(self,inarray):
        ...
        acts = [cu.array(inarray)]
        for layer in range(len(self._thetas)):
            # get previous activation and apply bias
            p_layer = cu.concatenate((cu.array([1]),acts[-1]))
            # apply transformation
            n_array = cu.matmul(p_layer,self._thetas[layer])
        ...
        if len(acts) != len(self._thetas)+1:
            ...
            for layer in range(-1,-len(acts),-1):
                ...
                if layer == -1:
                    ...
                    thetaT = cu.transpose(self._thetas[layer+1])
                    # getting the next cost
                    cost_next = deltas[layer+1]
                    # bring cost back
                    back_errr = (cu.matmul(cost_next,thetaT))[1:]
                ...
                preact = cu.concatenate((cu.array([1]),acts[layer-1]))
                ...
                delta_theta = cu.matmul(actT,deriM)
